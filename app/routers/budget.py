...
def create_budget(budget: schemas.BudgetCreate, db: Session = Depends(get_db)):
    ...
    ).first()      # Exclude soft-deleted budgets
    if existing_budget:
        raise HTTPException(
    ...
    soft_deleted_budget = reactivate_soft_deleted_budget(db, budget.user_id, budget.model_dump())
...
@router.delete("/{id}", status_code=status.HTTP_201_CREATED, response_model=schemas.BudgetOut)
