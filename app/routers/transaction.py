...
from app.utils import check_deleted, check_existence, check_ownership, get_transaction_by_id, get_user_budget, get_category_by_id, get_user_transaction
...
def get_specific_transaction(id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
...
def update_transaction(
    ...
    db.query(models.Transaction).filter(models.Transaction.id == id).update(transaction.model_dump(), synchronize_session=False)
    db.commit()
