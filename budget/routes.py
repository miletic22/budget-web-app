...
def budget():
    if request.method == "GET":
        user_budget = Budget.query.filter_by(user_id=1).first()
...
def category():
    if request.method == "GET":
        categories = Category.query.filter_by(budget_id=1).all()
        for i in categories:
            print(i.name)
        ...
        if form_type == "categories-update":
            ...
            for key, value in form_data.items():
                if key.startswith("category_") and key.endswith("_name"):
                    ...
                    category_name = value
                    category_amount = form_data.get(
                        f"category_{category_number}_amount", ""
                    )
...
def sign_up():
    if request.method == "POST":
        ...
        if user:
            ...
            flash(
                "Invalid username: must be longer than 2 characters.", category="error"
            )
        elif password1 != password2:
            flash("Passwords do not match.", category="error")
        elif len(password1) < 7:
            flash(
                "Invalid password: must be longer than 7 characters.", category="error"
            )
        else:
            new_user = User(
                username=username,
...
def transaction():
    ...
    if request.method == "POST":
        ...
        if not amount.replace(".", "", 1).isdigit():
            ...
            specific_category = category_query.filter(
                Budget.user_id == current_user.id, Category.name == category_name
            ).first()
            ...
            db.session.commit()
            flash(
                f'Added transaction under "{specific_category.name}" Category for ${amount}',
                category="success",
            )
